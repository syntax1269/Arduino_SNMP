...
If you're upgrading from any prior release (including the original Arduino_SNMP `v2.1.x`, or this fork's `v2.2.0` / `v3.0.0` / `v3.1.0` / `v3.1.1` / `v3.1.2` / `v3.1.3`), read **"What's New in v3.1.4"** immediately below. v3.1.4 is a single cumulative release that folds: (1) the v2.2.0 C-style string-model refactor, (2) the v3.0.0 critical BER TLV bug fixes (PR #60), (3) the 4-phase zero-heap deterministic-memory refactor, (4) v3.1.1 sketch-overridable tuning + SNMP_Sensor bug fixes, (5) v3.1.2 snmpTrapOID.0 RFC-3416 fix, (6) v3.1.3 ESP8266 auto-tune profile, and (7) v3.1.4 startup-heap ASNPool + narrowed SortableOID + universal OCTET=256 defaults into one tested tag.

---

## What's New in v3.1.4 (Cumulative: ALL changes since v2.1.0)

v3.1.4 is a single release combining every earlier in-tree milestone category. If you are upgrading from the historical `Arduino_SNMP` v2.1.x (or from any prior v2.2.0 / v3.0.0 / v3.1.x fork release) you are getting everything at once in this tag. Seven broad buckets of change rolled into v3.1.4:
...
```cpp
const char* getFirmwareVersion(void) { return LIBRARY_VERSION; }  // "3.1.4" from defs.h
...
Declared in [defs.h](file:///Users/syntax/Documents/trae_projects/SNMP_Agent/src/include/defs.h):
...
### Buffer Sizing
Fixed maximum sizes are declared in [defs.h](file:///Users/syntax/Documents/trae_projects/SNMP_Agent/src/include/defs.h):
