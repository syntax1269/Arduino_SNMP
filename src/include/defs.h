 ...
 *  envelopes) + INFLIGHT*3 (traps: envelope + OID + value each, decoded
 *  while a response is being built) + 24 (parse/response headroom —
 *  MEASURED worst tick on ESP-01: usedCountPeak 66 at TINY caps under
 *  sustained Cr6-walk + SET saturation with trap concurrency; the naive
 *  +10 left only margin 10, and the 23-handler hwtest campaign peaked
 *  at 75/76 — one slot from exhaustion).
 *
 ...
 *  derives 72. Sketches with MORE handlers automatically get MORE pool
 *  (a 40-handler deployment derives 100; the old fixed 76 starved it).
 *  Sketch-side SNMP_POOL_ASN_OBJECTS #define still wins (override).
 *  Floor: SNMP_POOL_ASN_OBJECTS >= 24 static_assert unchanged.
 *
 ...
 * ===================================================================== */
  ...
  #define SNMP_WORST_TICK_TRANSIENTS  ( ((SNMP_MAX_VARBINDS) + 2) * 3 + (SNMP_MAX_TRAPS_INFLIGHT) * 3 + 24 )
#endif
