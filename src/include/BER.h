...
#define SNMP_POOL_SLOT_SIZE 640
    ...
    static bool permFrozen;     /* v3.1.25: true once the startup baseline is frozen */
     ...
     * on failure logs and leaves the v3.1.x lazy path armed. Idempotent. */
    ...
    /* v3.1.25: one-shot safety net for sketches that never call
     ...
     * reuse that corrupted response OIDs on ESP-01 ('.1.3' truncation,
...
#endif
        /* v3.1.25: resetAll() is the single choke point every pool-reset caller
...
/* =====================================================================
 * POOL SLOT RIGHT-SIZE GUARDS (v3.1.25)
 * ---------------------------------------------------------------------
 ...
 * instead of stale headroom — before v3.1.25 the slot was 512/640 B while
 * the real largest object was 288/312 B, padding ~44%/49% of the arena.
