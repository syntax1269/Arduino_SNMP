    ...
    for(int n = 0; n < structure->valuesLen && state != DONE; n++){
        ...
        switch(state) {
            ...
            case VARBIND:
            {
                ...
                /* v3.1.24: LOUD REJECT on capacity overflow. Former behavior
                 ...
                 * response). Mirrors the v3.1.14 §4.2 community-cap policy:
                 * truncation replaced with explicit reject. */
                if(!this->emplace_back(oidClone, valueView)){
                    SNMP_LOGW("Request varbind #%d exceeds SNMP_MAX_VARBINDS (%d): REJECTING packet (was: silently truncated). Raise SNMP_MAX_VARBINDS via sketch #define before #include <SNMP_Agent.h>.\n",
